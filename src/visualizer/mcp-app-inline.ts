...
/**
 * Reads the Vite-built Phaser bundle from visualizer/dist at runtime and
...
function loadVisualizerBundle(wsPort: number, extensionPath: string, savedChar?: string): string | null {
    try {
        const distDir: string = path.join(extensionPath, 'visualizer', 'dist');
        const indexHtml: string = fs.readFileSync(path.join(distDir, 'index.html'), 'utf8');
        const match: RegExpMatchArray | null = indexHtml.match(/src="([^"]+index[^"]*\.js)"/);
        if (!match) {
            return null;
        }
        const jsRel: string = match[1].replace(/^\//, '');
        const jsCode: string = fs.readFileSync(path.join(distDir, jsRel), 'utf8');
        // Set globals before the module executes
        const charJson: string = savedChar ? JSON.stringify(savedChar) : 'undefined';
        return [
...
export function getVisualizerAppHtml(wsPort: number, extensionPath: string, savedChar?: string): string {
    ...
    const phaserBundle: string | null = loadVisualizerBundle(wsPort, extensionPath, savedChar);
...
<body>
  ...
  <!-- HUD bar -->
  <div class="hud">
    <div><span class="dot" id="dot"></span><strong>IronBee DevTools</strong><span id="status">Connecting…</span></div>
  ...
  ${phaserBundle
        ...
        : `<div class="no-build">Phaser bundle not found. Run <code>npm run build</code> inside visualizer first.</div>`}
