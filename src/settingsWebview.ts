...
export class SettingsWebviewProvider implements vscode.WebviewViewProvider {
    public static readonly viewType: string = 'ironbeeDevTools.settingsView';
    ...
    public resolveWebviewView(
        ...
        webviewView.webview.onDidReceiveMessage(
            async (data: { type: string; key: string; value: unknown }): Promise<void> => {
                switch (data.type) {
                    case 'updateSetting': {
                        const config: vscode.WorkspaceConfiguration =
                            vscode.workspace.getConfiguration('ironbeeDevTools');
                        // Platform toggles are project-scoped: write to the workspace when one is open
                        // (so each project picks its own platforms), otherwise fall back to global.
                        const isPlatformSetting: boolean = data.key.startsWith('platform.');
                        const hasWorkspace: boolean = (vscode.workspace.workspaceFolders ?? []).length > 0;
                        const target: vscode.ConfigurationTarget =
                            isPlatformSetting && hasWorkspace
                                ? vscode.ConfigurationTarget.Workspace
                                : vscode.ConfigurationTarget.Global;
                        await config.update(data.key, data.value, target);
                        break;
                    }
                    case 'openSettings': {
                        vscode.commands.executeCommand(
                            'workbench.action.openSettings',
                            '@ext:ironbee-ai.ironbee-devtools-vscode'
                        );
                        break;
                    }
                    case 'getSettings': {
        ...
        vscode.workspace.onDidChangeConfiguration((e: vscode.ConfigurationChangeEvent): void => {
            if (e.affectsConfiguration('ironbeeDevTools')) {
                this._sendSettings();
            }
        });
    }
    ...
    private _sendSettings(): void {
        if (this._view) {
            const config: vscode.WorkspaceConfiguration = vscode.workspace.getConfiguration('ironbeeDevTools');
            this._view.webview.postMessage({
                type: 'settings',
                settings: {
                    enable: config.get('enable'),
                    platform: {
                        browser: config.get('platform.browser.enable'),
                        node: config.get('platform.node.enable'),
                        backend: config.get('platform.backend.enable'),
                    },
                    browser: {
    ...
    private _getHtmlForWebview(webview: vscode.Webview): string {
        const iconUri: vscode.Uri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'images', 'icon.png'));
...
<head>
    ...
    <title>IronBee DevTools Settings</title>
    <style>
        :root {
...
<body>
    <div class="header">
        <img src="${iconUri}" alt="IronBee DevTools" class="header-icon">
        <div>
            <h1>IronBee DevTools</h1>
        </div>
    </div>
    ...
    <div class="section">
        ...
        <div class="setting-row">
            <div class="setting-info">
                ...
                <div class="setting-description">Enable or disable the IronBee DevTools extension</div>
            </div>
        ...
        <div class="setting-row">
            <div class="setting-info">
                <div class="setting-label">Browser Platform</div>
                <div class="setting-description">Playwright browser automation & visual debugging</div>
            </div>
            <div class="setting-control">
                <input type="checkbox" id="platform.browser.enable" checked>
            </div>
        </div>

        <div class="setting-row">
            <div class="setting-info">
                <div class="setting-label">Node Platform</div>
                <div class="setting-description">Non-blocking debugging of Node.js processes</div>
            </div>
            <div class="setting-control">
                <input type="checkbox" id="platform.node.enable">
            </div>
        </div>

        <div class="setting-row">
            <div class="setting-info">
                <div class="setting-label">Backend Platform</div>
                <div class="setting-description">HTTP/gRPC/GraphQL/WS + log capture + database verification</div>
            </div>
            <div class="setting-control">
                <input type="checkbox" id="platform.backend.enable">
            </div>
        </div>
    </div>
    ...
    <!-- Node Settings (used when the Node platform is enabled) -->
    <div class="section">
    ...
    <script>
        ...
        function updateUI(settings) {
            ...
            // Platforms (each enabled platform runs as its own MCP server)
            setCheckbox('platform.browser.enable', settings.platform?.browser);
            setCheckbox('platform.node.enable', settings.platform?.node);
            setCheckbox('platform.backend.enable', settings.platform?.backend);
